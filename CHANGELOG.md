...
- 运行规则收为三个运行文件：[入口](SKILL.md)、[编辑边界](references/editing-guide.md)和[示例](references/examples.md)；旧词表、场景包和固定判定链退出运行包，保留归档供历史复查。
- 默认按用户要求做最小必要编辑，保留抽象信息、否定范围、情态、交付条件和作者语气；程序代码块（含注释和文档字符串）默认逐字保护，代码围栏里的普通文案仍可按要求编辑。
- 未指明来源的权威论断默认保留归属并在必要时提示缺来源；用户明确要求删除时，才按编辑范围处理。
- 引号按用途判断：依据引用、文献、规范、界面文字、定义中的词和书名号标题按原文保留；叙述中的讲话可以清理口头语、填充词和重复，但保留事实、判断和情态。引号强调的关键词仍按上下文审查。
- 主集共 123 条（65 条待改善文本、58 条正常文本），其中 3 条覆盖引号用途；正向编辑样本共 7 条，要求至少 5 条有收益。
- 本地发布验收通过：Claude Opus 5 完成 136 条，Grok 4.6 完成 56 条定向验收；信息与任务硬失败均为零，正向收益各为 5/7。99 项工程测试通过。最终结果、格式恢复记录与覆盖限制见[评测记录](evals/results-v2.5.0.md)。本版尚未发布，全局安装未自动更新。

## [2.4.1] - 未发布 — 修正改写示例与保真指令

> 维护者已决定跳过本版独立发布。以下改动保留为 v2.5.0 的待整合材料；历史评测不代表 v2.5.0 已通过验证。

### 修复

- 修正 `references/examples.md`、`structures.md`、`scene-packs.md`、`positive-style.md` 中多处改写新增事实的示例。有些原文补入了示例所需的背景，有些改写删去了原文没有的数字、来源或动作；改写中的事实现在能回指配套原文。
- `examples.md` 的虚构跑分改用虚构产品名，删除补造的 CVE 编号；标注模式示例的默认输出改为正文。`structures.md` 的无源引用示例不再补写“Stanford 2023”来源。
- 发布前复核确认 SF-37 的旧输出误删了条件式交付动作。修正 `SKILL.md`、`phrases-zh.md`、`operation-manual.md` 和 `scene-packs.md` 的相关指令：去掉推销语气时，仍保留主体、动作、条件与情态。
- 修正“效率提升 → 快了多少、省了几个人”和操作手册中的同源示例，保留效率提高这一效果，不推测人数、耗时或成本。短语表要求数据和来源必须已由原文或作者提供。二元对比改写保留两边信息与否定范围，决定性与能力情态不随语气删改。

### 调整

- HUMAN 长文对照新增 2 篇固定 revision 的中文原作，共 10 篇、9 个作者组；历史文本 3 篇、现代文本 7 篇，中文原作 8 篇、英译中 2 篇。两篇新增文本仅机械抽取，分别采用 Public Domain 与 CC BY-SA 4.0。
- direct 场景覆盖 `docs`、`public-writing`、`status` 后，移除 HUMAN 代表性检查的临时 `known-gap` 放行。
- `positive-style.md` 的语域说明补充时代感：当代口语中突兀的旧式书面语按语域判断，怀旧、年代作品和刻意复古的文风保留；没有将这类词统一列为禁用词。观察来自 #14。
- `hard_metrics.py` 新增段落统计：段数、每段汉字和句数、最长段及列表行数。只报告数据，不增加风格阈值或修改硬判逻辑。
- README 简化文案，明确改写仍可能失真，并修正 `bounded` 与 `in-place` 的适用范围。AGENTS 修正开发源与本地运行安装包的关系。

### 验证与限制

- 9 月 4 日旧候选 `61062b89c06a` 完成双模型各 120 条改写；交叉 judge 实际覆盖 120 / 119 条，Grok 缺的 B-82 已由 Codex 静态补核。
- 9 月 10 日复核更正：旧候选 Grok 的 SF-16 风格未通过，SF-37 为 L1 失败。旧候选 SF 通过 Claude 53/63、Grok 49/63；L1 为 0/120、1/120；SNF 误杀为 2/57、1/57（后者含静态补核）。原始 judge 与失败输出保留，旧候选不能发布。
- 两轮修复各做 16 条双模型定向改写，静态复核均未通过。第二轮 `a42417b70c58` 的 Claude 在 SF-07、SF-36，Grok 在 SF-16、SF-39 各有 2 条 L1；未继续交叉 judge，当前候选不可发布。旧全量成绩不迁移到新规则。完整范围、裁决和模型记录见 [评测结果](evals/results-v2.4.1.md)。
- `mini` 未修改，不包含本次规则修复；HUMAN 仅做静态统计，不代表长文改写能力通过验证。
